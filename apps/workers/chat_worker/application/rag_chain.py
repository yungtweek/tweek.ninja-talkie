...
def doc_rank(d: Document) -> int:
    ...
    meta = d.metadata if isinstance(d.metadata, dict) else {}
    try:
        return int(meta.get("__orig_rank")) if meta.get("__orig_rank") is not None else 10**9
    except Exception:
        return 10**9
...
class RagPipeline:
    ...
    def __init__(
            ...
            *,
            ...
            search_type: WeaviateSearchType | None = None,
    ):
    ...
    def compress_docs(self, docs: List[Document], query: str):
        """
        ...
        if DC and EF:
            for th in (0.20, 0.10, 0.0):
                try:
                    ...
                    out = compressor.compress_documents(docs, query)
...
def make_rag_chain(llm: Optional[BaseLanguageModel] = None, settings: RagConfig | None = None,
